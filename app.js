const app = {
  ...
  updateCards() {
    ...
    const balanceHeroEl = document.getElementById('stat-balance-hero');
    const balanceCardEl = document.getElementById('stat-balance-card');
    const balanceValue = this.formatMoney(balance);
    const balanceClass = balance >= 0 ? 'card-value color-up' : 'card-value color-down';

    if (balanceHeroEl) {
      balanceHeroEl.innerText = balanceValue;
    }
    if (balanceCardEl) {
      balanceCardEl.innerText = balanceValue;
      balanceCardEl.className = balanceClass;
    }
